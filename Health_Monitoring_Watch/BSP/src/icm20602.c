/**
 * @file    icm20602.c
 * @brief   ICM20602 六轴传感器板级驱动: 引脚配置集中于 hardware_config.h,
...
#include "hal_delay.h"
#include "hal_gpio.h"
#include "hardware_config.h"   /* 引脚配置(ICM20602_SCL/SDA_*) */
#include <stdio.h>

/* ==================== 模拟I2C 位操作(经HAL层) ==================== */
#define SDA_H()     HAL_GPIO_WritePin(ICM20602_SDA_PORT, ICM20602_SDA_PIN, 1)
#define SDA_L()     HAL_GPIO_WritePin(ICM20602_SDA_PORT, ICM20602_SDA_PIN, 0)
#define SCL_H()     HAL_GPIO_WritePin(ICM20602_SCL_PORT, ICM20602_SCL_PIN, 1)
#define SCL_L()     HAL_GPIO_WritePin(ICM20602_SCL_PORT, ICM20602_SCL_PIN, 0)
#define SDA_READ()  HAL_GPIO_ReadPin(ICM20602_SDA_PORT, ICM20602_SDA_PIN)
...
{
    uint16_t t = 500;
    while (HAL_GPIO_ReadPin(ICM20602_SCL_PORT,
           ICM20602_SCL_PIN) == 0 && t--)
        ;
}
...
{
    /* SCL(PB0) / SDA(PC13): 开漏+上拉 (PC13内部上拉较弱, 建议外接4.7K上拉) */
    static const HAL_GPIO_Config_t s_scl = {
        .port  = ICM20602_SCL_PORT,
        .pin   = ICM20602_SCL_PIN,
        .clk   = ICM20602_SCL_GPIO_CLK,
        .mode  = GPIO_Mode_OUT,
        .otype = GPIO_OType_OD,
    ...
    static const HAL_GPIO_Config_t s_sda = {
        .port  = ICM20602_SDA_PORT,
        .pin   = ICM20602_SDA_PIN,
        .clk   = ICM20602_SDA_GPIO_CLK,
        .mode  = GPIO_Mode_OUT,
        .otype = GPIO_OType_OD,
        .speed = GPIO_Speed_100MHz,
        .pull  = GPIO_PuPd_UP,
